...
//					map_addflooritem(&it,count,sd->bl.m,sd->bl.x,sd->bl.y,0,0,0,0);
					map_addflooritem(&it,count,sd->bl.m,sd->bl.x,sd->bl.y,0,0,0,0,0); //ถ้าเซิฟรุ่นใหม่ๆ2017ใช้ตัวนี้
			}
		}
	}

//	log_pick_pc(sd, log_type, -amt, &sd->status.inventory[i]);
	log_pick_pc(sd, log_type, -amt, &sd->inventory.u.items_inventory[i]); //ถ้าเซิฟรุ่นใหม่ๆ2017ใช้ตัวนี้
	return 1;
}

...
/*				
				...
				clif_displaymessage(fd, "@afk ไม่สามารถใช้ได้ใน Map นี้");
				return 0;
				}
...
*/				
				if( pc_isdead(sd) ) {
				clif_displaymessage(fd, "@afk ไม่สามารถใช้ได้เมื่อตัวละครของคุณตาย");
				return -1;
				}
                ...
                clif_displaymessage(fd, "คุณไม่สามารถใช้ @afk ใน Map PVP และ GVG");
                return -1;}
...
ACMD_FUNC(itemmap)
{
	...
	if (!message || !*message || (get_type == 0 && sscanf(message, "\"%99[^\"]\" %d", item_name, &amount) < 1 && sscanf(message, "%99s %d", item_name, &amount) < 1))
	{
